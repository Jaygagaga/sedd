...
def get_loss_fn(noise, graph, train, sampling_eps=1e-3, lv=False):
    ...
    def loss_fn(model, batch, cond=None, t=None, perturbed_batch=None):
        ...
        """
        batch, target_mask, _ = _parse_batch(batch)

        if target_mask is not None:
            target_mask = target_mask.to(batch.device).bool()

        if t is None:
            if lv:
                raise NotImplementedError("Yeah I gotta do this later")
            else:
                t = (1 - sampling_eps) * torch.rand(batch.shape[0], device=batch.device) + sampling_eps #采样时间t
            
        sigma, dsigma = noise(t) #生成噪声总强度和加噪速率
        
        if perturbed_batch is None:
            perturbed_batch = graph.sample_transition(batch, sigma[:, None])
            if target_mask is not None:
                perturbed_batch = torch.where(target_mask, perturbed_batch, batch)
        ...
        if target_mask is not None:
            denom = target_mask.sum(dim=-1).clamp_min(1).to(loss.dtype)
            loss = loss / denom
