...
class Absorbing(Graph):
    ...
    def transp_transition(self, i, sigma): ## If the current state is MASK, add the extra mass for all possible previous tokens.
    ...
    def score_entropy(self, score, sigma, x, x0):
        ...
        other_ind = x0[rel_ind] #是mask的那些词

        # negative_term
        neg_term = ratio * torch.gather(score[rel_ind], -1, other_ind[..., None]).squeeze(-1)
        """ 先取出所有 rel_ind=True 的那些位置的 score
            这些位置上，每个位置都有一个长度为 V 的 score 向量
            other_ind 告诉你：每个位置真正对应的原 token 是哪个 id
            gather 就把“真实原 token 对应的那个 score”取出来"""
        #positive term
