...
class SEDD(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(self, indices, sigma):
        ...
        if self.scale_by_sigma:
            ...
            esigm1_log = torch.where(sigma < 0.5, torch.expm1(sigma), sigma.exp() - 1).log().to(x.dtype)[:, None, None] #标量， 【1，1，1】
            x = x - esigm1_log - np.log(x.shape[-1] - 1)# x[1, 512, 50258] this will be approximately averaged at 0 也就是对每个 batch 的所有 token、所有类别统一减去一个和 sigma 有关的偏移量。
            
        x = torch.scatter(x, -1, indices[..., None], torch.zeros_like(x[..., :1])) #在每个序列位置上，把当前 token 自己那一类的 log-score 强制设为 0。
        """ 因为这一项不是未知量，而是定义上就固定的。
            如果不手动钉死：
            网络可能输出一个不是 0 的值
            loss 又会把它往正确值拉
            等于浪费一部分容量去学一个本来就知道的答案"""
